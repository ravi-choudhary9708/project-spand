...
from app.engines.ai_remediation import get_remediation_playbook, calculate_priority_score
...
def run_full_scan(self, scan_id: str):
    """
    Main scan task. Orchestrates the full 17-step pipeline from DFD Level 1.
    """
    ...
    try:
        ...
        # Step 1: Asset Discovery - expand with subdomains
        self.update_state(state="PROGRESS", meta={"progress": 5, "step": "Asset Discovery"})
        expanded_domains = []
        for target in targets:
            if not any(c.isdigit() for c in target.split(".")[0]):  # Skip IPs
        ...
        for i, domain in enumerate(all_domains):
            ...
            try:
                ...
                # Process TLS data & calculate HNDL
                ...
                for cert_data in scan_data.get("certificates", []):
                    ...
                    cert_hndl = calculate_hndl_score(main_algorithm, main_key_size, 5.0, expires_at)
                ...
                # Calculate asset HNDL score
                asset_hndl = calculate_hndl_score(main_algorithm, main_key_size, 5.0, expires_at)
                ...
                # Create quantum vulnerability finding
                if is_quantum_vulnerable(main_algorithm):
                    finding = _create_finding(
                        db, asset.asset_id, FindingType.QUANTUM_VULNERABLE_ALGO,
                    ...
                    if finding:
                        ...
                        # Compliance mapping
                        violations = map_finding_to_compliance(main_algorithm, tls_data.get("tls_version", ""))
                        for v in violations:
                            tag = ComplianceTag(
        ...
        return {"status": "completed", "scan_id": scan_id, "assets": len(processed_assets)}
...
def _get_service_category(protocol: str) -> str:
    mapping = {
        ...
        "FTPS": "file_transfer", "SSH": "remote_access",
        "VPN": "vpn",
    }
...
def run_scheduled_rescans():
    """Trigger automated rescans for monitored assets."""
    logger.info("Running scheduled rescans...")
    # Implementation: query assets with monitoring enabled and re-trigger scans
    pass
